...
# ─── Default paths ────────────────────────────────────────────────────────────
CSV_DIR_DEFAULT = "/root/studiotj-data/prodigi-pricing"
PRODUCTS_DIR_DEFAULT = "/root/studiotj-website/content/products"
REPORT_PATH_DEFAULT = "/root/studiotj-website/scripts/sku-mapping-report.md"
...
def build_book_lookup(price_index: dict) -> dict:
    """
    Derive book SKUs from known pattern BOOK-FE-{size}-{HARD/SOFT}-MHK.
    Returns {(book_format, size_key): sku} for all keys found in price_index.
    """
    lookup = {}
    for sku in price_index:
        ...
        if len(parts) < 5:
            continue
        # BOOK-FE-{size}-{HARD/SOFT}-MHK
        try:
            book_type = parts[3].upper()  # HARD or SOFT
            size_key = parts[2].upper()
        except IndexError:
            continue
        if book_type == "HARD":
...
def run_update(files: list, price_index: dict, products_dir: Path,
    ...
    if unmatched:
        unmatched_path = products_dir.parent / "scripts" / "_unmatched.txt"
...
def main():
    ...
    ap.add_argument("--csv-dir", default=CSV_DIR_DEFAULT,
                    help=f"Directory containing Prodigi pricing CSVs (default: {CSV_DIR_DEFAULT})")
