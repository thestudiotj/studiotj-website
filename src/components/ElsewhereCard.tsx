...
interface ElsewhereCardProps {
  ...
  imageHeight?: number
  /** Eager mode for the homepage strip: skip viewport/onLoad fade-ins and
   *  force the image box to reserve space via CSS aspect-ratio, so the picture
   *  paints the moment it decodes (no layout-timing race). */
  eager?: boolean
}

export default function ElsewhereCard({ item, index, imageHeight, eager = false }: ElsewhereCardProps) {
  ...
  const aspectRatioCss = String(item.aspect_ratio > 0 ? item.aspect_ratio : 1)
  // Eager mode always pins aspect-ratio via CSS so the box has reserved space
  // before any JS layout or image decode lands.
  const imageStyle: React.CSSProperties = eager
    ? { aspectRatio: aspectRatioCss, ...(imageHeight ? { height: imageHeight } : {}) }
    : imageHeight
      ? { height: imageHeight }
        ...
        {isTextOnly ? (
          ...
          <div className="relative overflow-hidden" style={imageStyle}>
            ...
            <img
              ...
              decoding="async"
              onLoad={eager ? undefined : () => setImgLoaded(true)}
            />
            <IdentityDot identity={item.identity} />
          </div>
        )}
  ...
  // Eager: no viewport-driven fade-in, so the card paints immediately on hard
  // reload without depending on IntersectionObserver firing.
  if (eager) {
