...
The limits are real and worth knowing. The 11-point autofocus array clusters toward the centre of the frame and starts showing its age on moving subjects and in low light — fine for architecture and landscape work, frustrating for anything that doesn't stand still. Video caps at 1080p with built-in mono microphones and no external mic port, which is why the Pocket 4 covers the video side instead. Connectivity is Bluetooth-only via Nikon's SnapBridge app; image transfer to phone is slow, and you'll want a card reader for any volume. The LCD is fixed — no tilt, no touch — and charging needs the dedicated brick. The 95% viewfinder coverage means a sliver of the frame edge is guesswork.